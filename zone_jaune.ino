...
void readAndPublishTemperature() {
  ...
  if (!isnan(temperature)) {
    ...
    if (client.connected()) {
      client.publish("technique/sensors/temperature", String(temperature).c_str());
    } else {
      Serial.println("Erreur : Non connecté au broker MQTT !");
    }
  } else {
...
void readAndPublishHumidity() {
  ...
  if (!isnan(humidity)) {
    ...
    if (client.connected()) {
      client.publish("technique/sensors/humidite", String(humidity).c_str());
    } else {
      Serial.println("Erreur : Non connecté au broker MQTT !");
    }
  } else {
...
void readAndPublishAirQuality(int gasSensor) {
  ...
  if (client.connected()) {
    client.publish("technique/sensors/fumee", String(gasLevel).c_str());
  } else {
    Serial.println("Erreur : Non connecté au broker MQTT !");
  }
}
